...
* **Layer 2 (Native ARM64 `libchrome.so` Binary Patching)**:
  * **P3A Scheduler**: Overwrites conditional branch at `0x0abac4dc` with an unconditional branch (`b 0x0abac374`), immediately triggering the scheduler abort routine in `P3AService::InitScheduler`.
  * **Brave Stats**: Replaces the upload trigger branch at `0x0c3c9240` with an ARM64 `nop` (`0x1f 0x20 0x03 0xd5`), silencing `BraveStatsUpdater::Start`.
  * **Web Discovery Project (WDP)**: Patches branch at `0x0c408aa0` (`b 0x0c408aac`) to bypass search host/query extraction in `BraveSearchDefaultHostExtractor`.
